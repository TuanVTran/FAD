...
def exportSummary():
    ...
    classification_group = for_summary.calculate_ror_summary(np_end_date)
    for_summary.export_data()

    classification_summary = ClassificationSummary(classification_group)
...
def export_report_with_specs_data():
    parse_data()
    exportSummary()
